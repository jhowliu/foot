...
def training(data, cut_size=150, sample_size=10):
    ...
    for i in xrange(len(data)):
        ...
        #sample_idx, test_idx = Sampling(ple_size)
    ...
    for i in xrange(len(data)):
        ...
        f = envelope(label_dic, dictionary, training_set[i], num_std)
    ...
    for i in xrange(len(data)): 
        testing_feature.extend(envelope(label_dic, dictionary, testing_set[i], num_std)) 
    ...
    for i in range(len(testing_feature)): 
        if svm.predict(testing_feature[i]) == testing_label[i]:
            correct += 1
    return feature, training_label, float(correct) / len(testing_feature)
...
if __name__ == '__main__':
    ...
    f = []
    cut_s = []
    for cut_size in range(200,300,50):
